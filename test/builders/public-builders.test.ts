...
assert(
  selectWithId !== selectWithIdAndEmail,
  'select builder chaining should keep returning new instances',
);
...
assertThrows('order by items', () => {
  (db.selectFrom('users').select('id').orderBy as (...args: never[]) => unknown)();
});
...
assertThrows('insert rows', () => {
  (insertBase.values as (...args: never[]) => unknown)();
});
...
assert(
  insertOneRow !== insertTwoRows,
  'insert builder chaining should keep returning new instances',
);
...
assertThrows('update assignments', () => {
  (updateBase.set as (...args: never[]) => unknown)();
});
...
assert(
  updateWithName !== updateWithStatus,
  'update builder chaining should keep returning new instances',
);
